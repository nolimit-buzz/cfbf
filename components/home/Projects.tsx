...
import type { ProjectItem, ProjectsSection } from '@/lib/strapi-types';

type ProjectsProps = Omit<ProjectsSection, '__component'>;
...
}> = ({ project, index, capitalLabel, capacityLabel }) => {
...
export default function Projects({
  ...
  const router = useRouter();
  const defaultCategory = categories[0]?.label ?? 'All';
  const [viewMode, setViewMode] = useState(viewTabs[0]?.tabId ?? 'list');
  ...
  // The first category acts as the "show everything" option.
  const applyFilter = (value: string) =>
    value === defaultCategory ? projects : projects.filter(p => p.category === value);

  const filteredProjects = applyFilter(filter);

  const handleFilterChange = (newFilter: string) => {
    setFilter(newFilter);
    const nextProjects = applyFilter(newFilter);
    setExpandedProject(nextProjects[0]?.projectId ?? null);
  };
      ...
      <div className="container mx-auto px-6 relative z-20">
        <SectionHeader
          ...
          tabs={viewTabs.map((tab, idx) => ({
            ...
            label: tab.label ?? '',
            icon: VIEW_TAB_ICONS[tab.tabId ?? ''] ?? List
          }))}
          dark={true}
        />
        ...
        <div className="flex gap-4 mb-12 overflow-x-auto pb-4 no-scrollbar">
          {categories.map((cat, idx) => (
            <button
              key={cat.id ?? idx}
              onClick={() => cat.label && handleFilterChange(cat.label)}
              className={`px-6 py-2 rounded-full border text-xs tracking-wide transition-all duration-300 font-sans whitespace-nowrap ${filter === cat.label
            ...
            >
              {filteredProjects.map((p, i) => (
                <motion.div
                  ...
                  className={`bg-white/5 rounded-[6px] border border-white/10 shadow-sm overflow-hidden transition-all duration-300 ${expandedProject === p.projectId ? 'ring-1 ring-brand-accent/20 shadow-lg' : 'hover:bg-white/10 hover:border-brand-accent/30'}`}
                >
                  <div
                    onClick={() => setExpandedProject(expandedProject === p.projectId ? null : (p.projectId ?? null))}
                    className="flex flex-col md:flex-row items-start md:items-center gap-6 p-8 cursor-pointer interactive group"
                  >
                    <span className="text-xs text-brand-accent font-mono bg-brand-accent/10 px-2 py-1 rounded-[6px]">/ {p.projectId}</span>
                    <h3 className={`text-xl md:text-2xl font-bold flex-1 transition-colors ${expandedProject === p.projectId ? 'text-brand-accent' : 'text-white group-hover:text-brand-accent'}`}>
                      {p.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-6 md:gap-12 text-sm text-gray-400 font-sans uppercase tracking-wider">
                      ...
                      <span className={`px-4 py-1.5 border rounded-full text-xs font-bold transition-colors ${expandedProject === p.projectId ? 'bg-brand-accent text-brand-dark border-brand-accent' : 'bg-white/5 text-gray-400 border-white/10'}`}>
                        {p.category}
                      </span>
                    </div>

                    <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${expandedProject === p.projectId ? 'bg-brand-accent text-brand-dark' : 'bg-white/10 text-white group-hover:bg-brand-accent group-hover:text-brand-dark'}`}>
                      {expandedProject === p.projectId ? <Minus size={20} /> : <Plus size={20} />}
                    </div>
                  </div>
