...
import type { ProjectsNextStepsSection } from '@/lib/strapi-projects-types';

type ProjectsNextStepsProps = Omit<ProjectsNextStepsSection, '__component'>;

export default function ProjectsNextSteps({
  eyebrow,
  headingPartOne,
  headingItalic,
  links,
}: ProjectsNextStepsProps) {
  return (
    <>
      <section className="mt-24 pt-12 pb-6 bg-[#051F1A] text-white relative z-10 border-t border-white/5">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight">
            {headingPartOne}<span className="text-[#9BB7B1] italic font-serif">{headingItalic}</span>
          </h2>
        </motion.div>
      </section>
