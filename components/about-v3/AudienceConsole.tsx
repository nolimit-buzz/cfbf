...
import type { AudienceSection } from '@/lib/strapi-about-types';

type AudienceConsoleProps = Omit<AudienceSection, '__component'> & { className?: string };

/**
 * Persona icons are bespoke styling with no CMS field. Looked up by the tab
 * label the CMS supplies, falling back to position, then to the first icon.
 */
const PERSONA_ICONS: Record<string, React.ElementType> = {
  'Institutional Investor': ShieldCheck,
  'Project Developer': Zap,
  'Development Partner': Globe,
};
const ICON_ORDER: React.ElementType[] = [ShieldCheck, Zap, Globe];

/** A PDF target opens in a new tab; everything else is an in-app route. */
const isDownloadHref = (href?: string) => Boolean(href && /\.pdf(\?|$)/i.test(href));
...
export default function AudienceConsole({
  ...
  personas,
  className = "",
}: AudienceConsoleProps) {
  const [activeIndex, setActiveIndex] = useState(0);
  const [expandedIndex, setExpandedIndex] = useState<number | null>(0);
  const router = useRouter();

  const items = personas ?? [];
  const currentData = items[activeIndex];
  const ActiveIcon =
    (currentData?.tabLabel ? PERSONA_ICONS[currentData.tabLabel] : undefined) ??
    ICON_ORDER[activeIndex % ICON_ORDER.length];
  const currentIsDownload = isDownloadHref(currentData?.ctaHref);
              ...
              <div className="lg:col-span-7 space-y-6 text-left">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-brand-primary text-xs font-bold uppercase tracking-widest font-mono">
                    {currentData.tabLabel}{journeySuffix}
                  </span>
                </div>
                ...
                {currentData.ctaLabel && currentData.ctaHref && (
                  <div className="pt-4">
                    <button
                      onClick={() => handleCta(currentData.ctaHref!, currentIsDownload)}
                    ...
                    >
                      {currentIsDownload ? <Download size={14} /> : <FileText size={14} />}
