...
import type { NewsArticleItem, NewsArticleDetailSection } from '@/lib/strapi-news-types';

interface PageProps {
  params: Promise<{ id: string }>;
  articles: NewsArticleItem[];
  detailSection?: NewsArticleDetailSection;
}

export default function NewsDetailPage({ params, articles, detailSection }: PageProps) {
  const router = useRouter();
  const resolvedParams = use(params);
  const articleId = resolvedParams.id;

  const articleIndex = articles.findIndex(item => item.articleId === articleId);
  const article = articles[articleIndex];

  if (!article) {
    return (
      <div className="bg-[#FAFDFB] text-brand-dark min-h-screen flex flex-col items-center justify-center p-6 text-center">
        <h2 className="text-3xl font-bold mb-4 font-sans text-brand-dark">{detailSection?.notFoundTitle ?? 'Article Not Found'}</h2>
        <p className="text-gray-500 mb-8 font-sans">{detailSection?.notFoundBody ?? 'The news article you are looking for does not exist or has been moved.'}</p>
        <button
        ...
        >
          <ArrowLeft size={16} /> {detailSection?.notFoundCtaLabel ?? 'Back to News Hub'}
        </button>
      </div>
    );
  }
  ...
  const relatedArticles = articles.filter(item => item.articleId !== articleId).slice(0, 3);
  ...
  const handleCopyLink = () => {
    if (typeof window !== 'undefined') {
      ...
      alert(detailSection?.copyLinkAlert ?? "Article link copied to clipboard!");
    }
  };
  ...
  const jsonLd = {
    ...
    "image": [article.image],
    "datePublished": article.date ? new Date(article.date).toISOString().split('T')[0] : undefined,
    "author": [{ "@type": "Person", "name": article.author }],
    "publisher": {
      "@type": "Organization",
      "name": detailSection?.publisherName ?? "Climate Finance Blending Facility (CFBF)",
      "logo": { "@type": "ImageObject", "url": detailSection?.publisherLogoUrl ?? "https://climatesupportfacility.org/logo.png" }
    },
  ...
  return (
    <div className="bg-[#FAFDFB] text-brand-dark min-h-screen relative overflow-hidden">
      <title>{`${article.title}${detailSection?.titleSuffix ?? ' | News & Insights'}`}</title>
      ...
      <meta name="DC.subject" content={themes.map(t => t.label).join(', ')} />
      <meta name="DC.description" content={article.excerpt} />
      <meta name="DC.publisher" content={detailSection?.dcPublisher ?? "Climate Finance Blending Facility"} />
      <meta name="DC.language" content={detailSection?.dcLanguage ?? "en"} />
      <meta name="DC.date" content={article.date ? new Date(article.date).toISOString().split('T')[0] : ''} />
      <meta name="DC.type" content={detailSection?.dcType ?? "News Article"} />
      ...
      <GlassHero
        ...
        parent={{ label: detailSection?.breadcrumbParentLabel ?? 'news', href: '/news' }}
      >
        {/* Metadata row */}
      ...
      <article className="container mx-auto px-6 py-20 relative z-10">
        ...
        <div className="mb-12 flex items-center justify-between border-b border-gray-100 pb-5 max-w-5xl mx-auto">
          ...
          >
            <ArrowLeft size={16} /> {detailSection?.backLabel ?? 'Back to News Hub'}
          </button>
          <div className="text-gray-400 text-xs font-mono uppercase">
            {detailSection?.breadcrumbPrefix ?? '/ news / '}{(article.tag ?? '').toLowerCase().replace(' ', '-')}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 max-w-5xl mx-auto">
          ...
          <div className="lg:col-span-3 order-1 lg:order-1">
            <div className="sticky top-[110px] flex flex-col gap-10 text-left">
              ...
              <div>
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-4 font-mono">{detailSection?.postedByLabel ?? 'Posted by'}</h4>
              ...
              <div>
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-4 font-mono">{detailSection?.themesLabel ?? 'Themes'}</h4>
                <div className="flex flex-wrap gap-2">
                  {themes.map((theme, idx) => (
                    <span key={theme.id ?? idx} className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider border rounded-[6px] border-gray-200/60 text-gray-500 bg-white">
                      {theme.label}
                    </span>
                  ))}
                </div>
              ...
              <div className="border-l-2 border-brand-accent pl-4 py-1">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-3 font-mono">{detailSection?.contextLabel ?? 'Context'}</h4>
                  ...
                  return (
                    <figure key={idx} className="my-12">
                      <div className="rounded-[6px] overflow-hidden border border-gray-200/40 shadow-md aspect-video max-h-[420px] w-full">
                        <img src={item.url} alt={item.url_alt_text || item.text} className="w-full h-full object-cover" />
                      </div>
                      {item.caption && (
            ...
            <div className="mt-16 pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <span className="text-sm font-semibold flex items-center gap-2 text-gray-400 font-sans">
                  <Share2 size={16} /> {detailSection?.shareLabel ?? 'Share'}
                </span>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    aria-label={detailSection?.shareLinkedinAriaLabel ?? "Share on LinkedIn"}
                  >
                    <Linkedin size={14} />
                  </button>
                  <button
                    ...
                    className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center bg-white text-gray-500 hover:text-brand-primary hover:border-brand-primary transition-all focus:outline-none interactive"
                    aria-label={detailSection?.shareTwitterAriaLabel ?? "Share on Twitter"}
                  >
                    <Twitter size={14} />
                  </button>
                  <button
                    ...
                    className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center bg-white text-gray-500 hover:text-brand-primary hover:border-brand-primary transition-all focus:outline-none interactive"
                    aria-label={detailSection?.shareFacebookAriaLabel ?? "Share on Facebook"}
                  >
                    <Facebook size={14} />
                  </button>
                  <button
                    ...
                    className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center bg-white text-gray-500 hover:text-brand-primary hover:border-brand-primary transition-all focus:outline-none interactive"
                    aria-label={detailSection?.copyLinkAriaLabel ?? "Copy Link"}
                  >
                    <Link2 size={14} />
                  </button>
                </div>
              ...
              <div className="text-xs text-gray-400 font-sans font-light">
                {detailSection?.publishedInPrefix ?? 'Published in '}{article.tag} · {article.date}
              </div>
            </div>
            ...
            <div className="mt-12 grid grid-cols-2 gap-4 border-t border-gray-100 pt-8">
              {prevArticle ? (
                <Link href={`/news/${prevArticle.articleId}`} className="flex flex-col items-start gap-1 p-4 rounded-[6px] border border-gray-100 hover:border-brand-primary/20 hover:bg-gray-50/50 transition-all text-left group">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1 font-mono">
                    <ChevronLeft size={12} className="group-hover:-translate-x-0.5 transition-transform" /> {detailSection?.previousArticleLabel ?? 'Previous Article'}
                  </span>
                ...
                <div className="opacity-20 border border-dashed border-gray-100 p-4 rounded-[6px] flex flex-col items-start">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider font-mono">{detailSection?.previousArticleLabel ?? 'Previous Article'}</span>
                  <span className="text-sm font-light text-gray-400 font-sans mt-1">{detailSection?.firstArticleLabel ?? 'First article'}</span>
                </div>
              )}
              ...
              {nextArticle ? (
                <Link href={`/news/${nextArticle.articleId}`} className="flex flex-col items-end gap-1 p-4 rounded-[6px] border border-gray-100 hover:border-brand-primary/20 hover:bg-gray-50/50 transition-all text-right group">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1 font-mono">
                    {detailSection?.nextArticleLabel ?? 'Next Article'} <ChevronRight size={12} className="group-hover:translate-x-0.5 transition-transform" />
                  </span>
                  <span className="text-sm font-semibold text-brand-dark group-hover:text-brand-primary transition-colors line-clamp-1 font-sans mt-1">
                    {nextArticle.title}
                  </span>
                ...
                <div className="opacity-20 border border-dashed border-gray-100 p-4 rounded-[6px] flex flex-col items-end">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider font-mono">{detailSection?.nextArticleLabel ?? 'Next Article'}</span>
                  <span className="text-sm font-light text-gray-400 font-sans mt-1">{detailSection?.latestArticleLabel ?? 'Latest article'}</span>
                </div>
              )}
            </div>
      ...
      <section className="bg-gray-50 border-t border-gray-200/50 py-24 relative z-10 text-left">
        <div className="container mx-auto px-6 max-w-5xl">
          <h3 className="text-2xl md:text-3xl font-bold text-brand-dark mb-10 font-sans tracking-tight">
            {detailSection?.relatedHeadingPartOne ?? 'Related'} <span className="text-[#7C9590]">{detailSection?.relatedHeadingHighlight ?? 'updates'}</span>
          </h3>
              ...
              >
                ...
                <div className="flex flex-col flex-1 justify-between">
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-1.5 text-xs text-brand-accent font-bold uppercase tracking-wider font-sans group-hover:text-brand-primary transition-colors">
                    {detailSection?.relatedCtaLabel ?? 'Read Article'} <ArrowRight size={12} className="group-hover:translate-x-0.5 transition-transform" />
                  </div>
                </div>
              </Link>
            ))}
