...
import { LayoutGrid, List, Calendar, ArrowRight, Tag } from 'lucide-react';
...
interface NewsArchiveClientProps {
  ...
  articlesSection?: NewsArticlesSection;
...
export default function NewsArchiveClient({
  ...
  const articles: NewsArticleItem[] = articlesSection?.articles ?? [];
  const viewTabs = listing?.viewTabs ?? [];
  const categories = listing?.categories ?? [];

  const [viewMode, setViewMode] = useState(viewTabs[0]?.tabId ?? 'grid');
  ...
  return (
    <div className="bg-[#FAFDFB] text-brand-dark min-h-screen relative overflow-hidden">
      ...
      <GlassHero
        title={
          <>
            {hero?.headingPartOne ?? 'News &'} <span className="text-[#9BB7B1]">{hero?.headingHighlight ?? 'media center'}</span>
          </>
        }
        subtitle={hero?.subtitle ?? 'Stay updated on the green transition'}
        bgImage={hero?.bgImage ?? 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?q=80&w=1200&auto=format&fit=crop'}
        currentPage="news"
      >
        <motion.div
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between text-left">
                    ...
                    <div className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-brand-primary group-hover:text-brand-dark transition-colors mt-2">
                      <span>{hero?.cardCtaLabel ?? 'Read Article'}</span>
          ...
          <div className="flex items-center justify-between md:justify-end gap-6 mt-2 pb-2">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                aria-label={hero?.prevAriaLabel ?? 'Previous articles'}
              >
              ...
              <button
                ...
                aria-label={hero?.nextAriaLabel ?? 'Next articles'}
              >
                <ArrowRight size={16} />
              </button>
            </div>
      ...
      <div data-rag-chunk="news-archive-container" className="container mx-auto px-6 py-24 relative z-10">
        ...
        <SectionHeader
          sub={listing?.sectionSub ?? 'Latest Updates'}
          title={listing?.sectionTitle ?? 'Insights, announcements and press statements'}
          activeTab={viewMode}
          onTabChange={setViewMode}
          tabs={viewTabs.map(tab => ({
            id: tab.tabId ?? 'grid',
            label: tab.label ?? '',
            icon: ICON_MAP[tab.tabId ?? 'grid'] ?? LayoutGrid,
          }))}
          dark={false}
        />
        ...
        <div className="flex gap-3 mb-16 overflow-x-auto pb-4 no-scrollbar">
          {categories.map(cat => (
            <button
              key={cat.id}
              onClick={() => setActiveFilter(cat.label ?? 'All')}
                  ...
                  >
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      ...
                      <div className="pt-4 border-t border-gray-100 flex items-center justify-between text-brand-accent font-bold text-xs uppercase tracking-wider group-hover:text-brand-primary transition-colors font-sans">
                        <span>{articlesSection?.gridCtaLabel ?? 'Read Article'}</span>
      ...
      <section className="pt-12 pb-6 bg-[#051F1A] text-white relative z-10 border-t border-white/5">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[#81C34D] text-xs font-semibold tracking-[0.2em] uppercase font-mono">{nextSteps?.eyebrow ?? 'Next steps'}</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight">
            {nextSteps?.headingPartOne ?? 'Explore the'} <span className="text-[#9BB7B1] italic font-serif">{nextSteps?.headingItalic ?? 'facility portal'}</span>
          </h2>
        </motion.div>
      </section>
